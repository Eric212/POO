...
public class PrincipalBicicleta {
    ...
    public static void consultaReferencia(int referencia){
        for(int j=0;j<baseDatos.length;j++){
            if(baseDatos[j]!=null){
                if(referencia==baseDatos[j].getReferencia()){
                    System.out.printf("\tDatos Bicicleta\n"+"---------------------------------\n"+"Referencia\t\t"+"%1d\n"+"Marca\t\t\t"+"%2s\n"+"Modelo\t\t\t"+"%2s\n"+"Peso\t\t\t"+"%2.1f"+"Kg\n"+"Pulgadas\t\t"+"%2.1f"+"\"\n"+"Motor\t\t\t"+"%2s\n"+"Fecha de fabricacion\t"+"%2tD\n"+"Precio\t\t\t"+"%2.2f"+"€\n"+"Existencias\t\t"+"%1d\n",baseDatos[j].getExistencias(),baseDatos[j].getMarca(),baseDatos[j].getModelo(),baseDatos[j].getPeso(),baseDatos[j].getRuedasPulgadas(),baseDatos[j].getMotor(),baseDatos[j].getFechaDeFabricacion().getTime(),baseDatos[j].getPrecio(),baseDatos[j].getExistencias());
